...
abstract class Model
{
    ...
    public function validate()
    {
        foreach ($this->rules() as $attribute => $rules) {
            ...
            foreach ($rules as $rule) {
                ...
                if ($ruleName === self::RULE_MATCH && strlen($value) && $value !== $this->{$rule['match']}) {
    ...
    public function errorMessages()
    {
        return [
            ...
            self::RULE_MIN => 'Minimun length must be {min}!',
            self::RULE_MAX => 'Maximun length must be {max}!',
            self::RULE_MATCH => 'This field must match {match}!',
            self::RULE_UNIQUE => '{field} already exist!',
        ];
    }
